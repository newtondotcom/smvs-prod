    ...
    for j in range(len(tab)):
        if retenue > 0:
            ...
            retenue = group_words_based_on_threshold(
                tab, new_tab, seuil, j, moyenne_time, moyenne_length
            )
...
def write_ass_file_aligned(file: TextIO, position):
    ...
    file.write(
        "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColor, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding, WrapStyle\n"
    )
    for j in styles:
        file.write(j)
    file.write("\n")
    ...
    for s in new_tab:
        ...
        file.write(
            f"Dialogue: 0,{format_seconds_to_hhmmss(globalstart)},{format_seconds_to_hhmmss(globalend)},{style},,50,50,20,fx,{localtext}\n"
        )
...
def write_ass_file_non_aligned(contents, file: TextIO):
    ...
    for s in contents:
        ...
        file.write(
            f"Dialogue: 0,{format_seconds_to_hhmmss(s['start'])},{format_seconds_to_hhmmss(s['end'])},,,50,50,20,fx,{s['text'].strip().replace('-->', '->')}\n"
        )
...
def process_video(path_in, path_out, emoji, lsilence, isVideoAligned, position):
    ...
    ass_path = os.path.join(
        "temp", f"{extract_filename_without_extension(path_in)}.ass"
    )  # Define ASS file path

    audio_path = extract_audio_from_videos([path_in])[
        path_in
    ]  # Get audio file path from video

    words, langage, time_transcription, time_alignment = get_transcribe(
        audio_path
    )  # Transcribe audio to extract words

    time_encoding = 0
    if isVideoAligned:
        time_encoding = video_aligned(
            words, ass_path, emoji, path_in, path_out, lsilence, position, langage
        )  # Process aligned video
    else:
        time_encoding = video_non_aligned(
            words, ass_path, emoji, path_in, path_out
        )  # Process non-aligned video
