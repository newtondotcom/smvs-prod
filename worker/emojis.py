...
def overlay_images_on_video(
    in_path, out_path, width, height, ass, position, font_size_pt, emojis_list=None
):
    """
    ...
    if emojis_list is not None:
        ...
        emojis_list = [
            (emojis_dir + image + ".png", start_time, end_time, offset_multiline)
            for image, start_time, end_time, offset_multiline in emojis_list
        ]

        for idx, (image, start_time, end_time, offset_multiline) in enumerate(
            emojis_list
        ):
...
def fetch_similar_emojis(array, language):
    ...
    for word_group in array:
        ...
        if current_iteration_found_emoji:
            entry = [
                current_iteration_found_emoji_number,
                word_group[0],
                word_group[1],
                1 if len(sentence) > 3 else 0,
            ]
